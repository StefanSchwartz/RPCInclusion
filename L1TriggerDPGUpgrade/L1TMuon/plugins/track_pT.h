...
void rpc_tuplizer::three_track_pt(vector<vector<int>> csc_hits, 
	...
	if ( dr > 0.1 ) {
	  ...
	  csc_rpc_match[nTrk][iLct][RpcTrkId_]      = -999;
          PtTrk_reco_rpc_csc[nTrk][iLct][RpcTrkId_] = -999;
	  continue;
	}
	...
	csc_rpc_match[nTrk][iLct][RpcTrkId_] = 1;
	...
	if (printLevel > 3) cout << "    Best Pt match = " << best_pt << endl;
	
	// store in branch
	PtTrk_reco_rpc_csc[nTrk][iLct][RpcTrkId_] = best_pt;
