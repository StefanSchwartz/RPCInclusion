void twoHit3(){
	...
	Int_t Two_Hit_Pt3;
	Int_t Two_Hit_Match3;
	Int_t Two_Hit_Match_Front3;
	Int_t Two_Hit_Match_Rear3;
	Double_t xbins[5] = {10,20,40,60,200};

	//Creating histograms of the data
	TH1F *h1 = new TH1F("h1","Two Hit trkPt distribution",4,xbins);
	TH1F *h2 = new TH1F("h2","Two_Hit_Match3 distribution",4,xbins);
	TH1F *h3 = new TH1F("h3","Two_Hit_Match_Front3 distribution",4,xbins);
	TH1F *h4 = new TH1F("h4","Two_Hit_Match_Rear3 distribution",4,xbins);

	//Creating pointers to the appropriate variables
	mytree->SetBranchAddress("Two_Hit_Pt3",&Two_Hit_Pt3);
	mytree->SetBranchAddress("Two_Hit_Match3",&Two_Hit_Match3);
	mytree->SetBranchAddress("Two_Hit_Match_Front3",&Two_Hit_Match_Front3);
	mytree->SetBranchAddress("Two_Hit_Match_Rear3",&Two_Hit_Match_Rear3);	
	...
	for (Int_t i = 0; i < nentries; i++){
		mytree->GetEntry(i);
		h1->Fill(Two_Hit_Pt3);
		h2->Fill(Two_Hit_Match3);
		h3->Fill(Two_Hit_Match_Front3);
		h4->Fill(Two_Hit_Match_Rear3);
	}
	...
	TFile *OutFile = TFile::Open("Two Hit Comparison 3.root","RECREATE");
	c1->Write();
	c1->Print("Two Hit Comparison 3.pdf");
	OutFile->Close();
	InFile->Close();
}
