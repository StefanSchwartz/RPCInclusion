...
void threeHit(){
	...
	Int_t Three_Hit_Match_Front;
	Int_t Three_Hit_Match_Rear;
	Double_t xbins[5] = {10,20,40,60,200};

	//Creating histograms of the data
	TH1F *h1 = new TH1F("h1","Three Hit trkPt distribution",4,xbins);
	TH1F *h2 = new TH1F("h2","Three_Hit_Match distribution",4,xbins);
	TH1F *h3 = new TH1F("h3","Three_Hit_Match_Front distribution",4,xbins);
	TH1F *h4 = new TH1F("h4","Three_Hit_Match_Rear distribution",4,xbins);
	...
	mytree->SetBranchAddress("Three_Hit_Match_Front",&Three_Hit_Match_Front);
	mytree->SetBranchAddress("Three_Hit_Match_Rear",&Three_Hit_Match_Rear);	
	...
	for (Int_t i = 0; i < nentries; i++){
		mytree->GetEntry(i);
		h1->Fill(Three_Hit_pt);
		h2->Fill(Three_Hit_Match);
		h3->Fill(Three_Hit_Match_Front);
		h4->Fill(Three_Hit_Match_Rear);
	}
	...
	leg->AddEntry(eff_both,"Front and Rear Matches","l");
	...
	TFile *OutFile = TFile::Open("Three Hit Comparison.root","RECREATE");
	c1->Write();
	c1->Print("Three Hit Comparison.pdf");
	OutFile->Close();
	InFile->Close();
}
